...
from .dialogos import mostrar_alerta, mostrar_confirmacion, mostrar_mensaje, mostrar_exito
import asyncio


def guardar_emisor(page, form, id_edicion, dialog_emisor, recargar_callback, limpiar_id_callback, exito_callback,):
    ...
    def ejecutar_guardado():
        ...
        if id_edicion is None:
            if Emisor.existe_nit(form.txt_nit.value):
                mostrar_mensaje(
                    page, 
                    f"El NIT {form.txt_nit.value} ya se encuentra registrado"
                )
                return

            Emisor.guardar(
                form.txt_nombre_comercial.value, 
                form.txt_razon_social.value,
                form.txt_nit.value, 
                form.txt_dui.value, 
                form.txt_nrc.value,
                int(form.ddl_tamaño_contribuyente.value), 
                form.txt_actividad.value,
                form.txt_telefono.value, 
                form.txt_correo.value, 
                form.txt_direccion.value
            )
        ...
        page.run_task(mostrar_exito_despues)
